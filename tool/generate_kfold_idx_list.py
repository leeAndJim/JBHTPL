...
def gen_kfold_txt(dataset, p=10, k=5):
    ...
    NKSID_class_slices = [0, 203, 491, 511, 1462, 1574, 1689, 2523]
    slices_list = []
    ...
    if dataset == 'KLSG':
        ...
        for i in range(len(LTSID_class_slices) - 1):
            arr = np.arange(LTSID_class_slices[i], LTSID_class_slices[i+1], dtype=np.int32)
            slices_list.append(arr)
    elif dataset == 'FLSMDD':
...
if __name__ == '__main__':
    args = parse_args()
    assert args.dataset in ['KLSG', 'FLSMDD', 'NKSID_part']
