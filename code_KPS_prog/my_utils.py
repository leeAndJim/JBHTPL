...
from model.JigModel import BasicConv

# from sampler import BalancedDatasetSampler

now_dir = os.path.dirname(__file__)
tb_dir = os.path.join(now_dir, 'output\\tensorboard\\')
print(tb_dir)
...
class FC(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.conv_block3 = nn.Sequential(
            BasicConv(self.num_ftrs, num_inputs, kernel_size=1, stride=1, padding=0, relu=True),
            BasicConv(num_inputs, self.num_ftrs//2, kernel_size=3, stride=1, padding=1, relu=True)
        )
        ...
        self.conv_block2 = nn.Sequential(
            BasicConv(self.num_ftrs//2, num_inputs, kernel_size=1, stride=1, padding=0, relu=True),
            BasicConv(num_inputs, self.num_ftrs//2, kernel_size=3, stride=1, padding=1, relu=True)
        )
        ...
        self.conv_block1 = nn.Sequential(
            BasicConv(self.num_ftrs//4, num_inputs, kernel_size=1, stride=1, padding=0, relu=True),
            BasicConv(num_inputs, self.num_ftrs//2, kernel_size=3, stride=1, padding=1, relu=True)
        )
    ...
    def forward(self, x):
        xs1, xs2, xs3 = x[0], x[1], x[2]

        # xs1 = self.conv_block1(xs1)
        # xs2 = self.conv_block2(xs2)
        # xs3 = self.conv_block3(xs3)
...
class Net(nn.Module):
    def __init__(self, model, nb_classes, dim_feats):
        ...
        self.Linear_layer = nn.Linear(dim_feats, nb_classes)  # 加上一层参数修改好的全连接层，例如修改为11层
...
def model_fc_fix(model, nb_classes, device=None):
    ...
    # model = Net(model, nb_classes, dim_feats)
    return model
...
def get_kfold_img_idx(p, k, dataset, sample_type=None):  
    ...
    if sample_type == 'balance':
        ...
        NKSID_balance_num = 66
        NKSID_part_balance_num = 66
        ...
        NKSID_part_class_slices = [0, 203, 491, 511, 1462, 1574, 1689, 2523]
    ...
    if sample_type:
        if sample_type == 'uniform':
            ...
            if dataset == 'KLSG':
                ...
                balance_num = NKSID_part_balance_num
                class_slices = NKSID_part_class_slices
            else:
...
def get_kfold_img_iters(batch_size, data_dir, train_idx, val_idx, mean, std):
    ...
    train_transform = transforms.Compose([
        ...
        # transforms.ColorJitter(contrast=(1, 9)),
    ...
    val_iter = torch.utils.data.DataLoader(val_imgs, 
                                            batch_size=batch_size, 
                                            num_workers=4)
    per_cls_weights = torch.tensor(train_imgs.get_cls_num_list(), device='cuda')
    per_cls_weights = per_cls_weights.sum() / per_cls_weights
    per_cls_weights = per_cls_weights / per_cls_weights.max()
    return train_iter, val_iter, train_imgs.get_cls_num_list(), per_cls_weights
...
'''
def train(train_iter, test_iter, net, loss_kps, loss_ce, optimizer, device, num_epochs):
    net = net.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        train_acc_sum1, train_acc_sum2, train_acc_sum3 = 0.0, 0.0, 0.0
        for X, y in train_iter:
            X = X.to(device)
            y = y.to(device)
            y_hat, _, _ = net(X, block=[4,4,0,0])
            # l1 = loss_ce(y_hat, y)
            l1 = loss_kps(y_hat, y)
            optimizer.zero_grad()
            l1.backward()
            ...
            train_l_sum += l2.cpu().item()
            train_acc_sum += (y_hat.argmax(dim=1) == y).sum().cpu().item()
            train_acc_sum2 += (y_hat.argmax(dim=1) == y).sum().cpu().item()
            n += y.shape[0]
            batch_count += 1

            _, _, y_hat = net(X, block=[0,0,0,0])
            # l3 = loss_ce(y_hat, y)
            l3 = loss_kps(y_hat, y)
            optimizer.zero_grad()
            l3.backward()
            ...
            train_l_sum += l3.cpu().item()
            train_acc_sum += (y_hat.argmax(dim=1) == y).sum().cpu().item()
            train_acc_sum3 += (y_hat.argmax(dim=1) == y).sum().cpu().item()
            n += y.shape[0]
            batch_count += 1

        test_acc = evaluate_accuracy(test_iter, net)
        # if record_acc <= test_acc:
        #     record_acc = test_acc
        #     model = net
        ...
        print('epoch %d, loss %.4f, train acc %.3f, train_acc1 %.3f, train_acc2 %.3f, train_acc3 %.3f, test acc %.3f, time %.1f sec'
              % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, train_acc_sum1 / n, train_acc_sum2 / n, train_acc_sum3 / n, test_acc, time.time() - start))
    return net
...
def train_mul(train_iter, backbone, fc, loss, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            X = X.to(device)
            y = y.to(device)
            backbone.fc_training = True
            features = backbone(X)
            backbone.fc_training = False
            y_hat = fc(features)
            l = loss(y_hat, y) 
            optimizer.zero_grad()
            l.backward()
...
def evaluate_gmean_optional(data_iter, backbone, fcs, method, fusion_net=None, device=None, if_get_y=False, if_get_logits=False):
    ...
    with torch.no_grad():
        for X, y in data_iter: 
            ...
            if method == 'soft_voting' and if_get_logits:
